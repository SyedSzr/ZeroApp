...
function ExploreScreen({ exploreCategory }) {
  const { openDetail, go, liveApps, liveCats, t } = useApp();
  ...
  const activeCat = activeTabId === 'all'
    ? null
    : homeCategories.find(c => c.id === activeTabId);
                      ...
                      <div className="flex items-center gap-3 mt-3 px-1">
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-muted text-[11px] font-medium">{app.category}</span>
            ...
            <div className="mb-2">
              <div className="px-4 mb-4">
                <span className="text-white font-black text-xl tracking-tight">
                  {activeCat ? `${t('all')} ${t('cat_' + activeCat.id)} ${t('apps')}` : t('all_apps')}
                  <span className="text-muted text-sm font-normal ml-2">({listApps.length})</span>
                </span>
              </div>
                  ...
                  <button key={app.id} onClick={() => openDetail(app)}
                    ...
                    <div className="flex-1 min-w-0 text-left">
                      ...
                      <div className="text-muted text-[11px] font-medium mt-1 truncate">
                        {(app.tags || []).slice(0,3).map((t_tag,i) => (
                      ...
                      <div className="flex items-center gap-1.5 mt-1.5">
                        ...
                        <span className="text-muted opacity-40">·</span>
                        <span className="text-muted text-xs font-medium uppercase tracking-tighter">{app.category}</span>
                      </div>
                    </div>
